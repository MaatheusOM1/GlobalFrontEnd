...
export default function CadastroDoencas() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/controleDoencasTransmissiveis/cadastrar', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert('Doença transmissivel cadastrada com sucesso!');
      ...
      setErroAtualizacao('Erro ao cadastrar doenca transmissivel. Tente novamente mais tarde.');
    }
  };

  return (
    <div className={styles.container}>
      <h1 className={styles.heading}>Cadastro de Doenças Transmissíveis</h1>
