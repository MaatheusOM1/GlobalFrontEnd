...
export default function CadastroProblemaSaude() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/problemas-saude/cadastrar', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert('Problema de Saude cadastrado com sucesso!');
      ...
      setErroAtualizacao('Erro ao cadastrar problema de saude. Tente novamente mais tarde.');
    }
  };

  return (
    <div className={styles.container}>
      <h1 className={styles.heading}>Cadastro de Problema de Saúde</h1>
